...
ggVennDiagram(x) + scale_fill_gradient(low="grey90", high="orange") + scale_x_continuous(expand = expansion(mult = .2))
```
...
length(methods)
#colormap <- setNames(c(brewer.pal(length(methods), "Paired"), "#999999"), c(methods, "NA"))
colormap <- setNames(c(brewer.pal(length(methods[1:12]), "Paired"), 
                       brewer.pal(length(methods[13:length(methods)]), "Pastel1"),
                       "#999999"), 
                     c(methods, "NA"))
findings %>%
